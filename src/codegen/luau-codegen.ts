...
export interface CodegenOptions {
  indentStr?: string;
  sourceFile?: string;
  /** Omit the "-- Auto-generated by rbx-tsx" header block */
  silent?: boolean;
  /** Luau hot-comments (e.g. "--!native") emitted as the first lines */
  directives?: string[];
 ...
 * locals can be emitted as `const`. Name-based and conservative: a name
 * reassigned in any scope keeps `local` everywhere. */
function collectAssignedNames(node: unknown, out: Set<string>): void {
  if (!node || typeof node !== "object") return;
  if (Array.isArray(node)) {
    for (const item of node) collectAssignedNames(item, out);
    return;
  }
  const n = node as { type?: string; target?: { type?: string; name?: string } };
  if (
    (n.type === "assignment" || n.type === "compound-assignment") &&
    n.target?.type === "identifier" &&
    n.target.name
  ) {
    out.add(n.target.name);
  }
  for (const value of Object.values(node)) collectAssignedNames(value, out);
}
...
export function generateLuau(
  ...
  if (options.directives && options.directives.length > 0) {
    lines.push(...options.directives);
    if (!options.silent || options.directiveGap) {
      lines.push("");
    }
  }
  if (!options.silent) {
  ...
  switch (stmt.type) {
    ...
    case "multi-local": {
      ...
      const keyword =
        ...
        stmt.names.every((n) => !ctx.assignedNames.has(n))
          ? "const"
          : "local";
      lines.push(`${t}${keyword} ${names} = ${values}`);
      break;
    }
    ...
    case "comment": {
      ...
      if (hasNewline) {
        ...
        lines.push(stmt.text === "" ? "" : `${t}-- ${stmt.text}`);
      }
      break;
    }
