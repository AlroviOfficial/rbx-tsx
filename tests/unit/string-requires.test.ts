...
describe("instancePathToStringRequire", () => {
  ...
  test("returns null for chains without a module segment", () => {
    expect(instancePathToStringRequire("script")).toBeNull();
    expect(instancePathToStringRequire("script.Parent")).toBeNull();
    expect(instancePathToStringRequire("script.Parent.Parent")).toBeNull();
  });
});
...
describe("tree-relative requires from file-level aliases", () => {
  test("emits a tree-relative chain when the importer has a file-level alias", () => {
    const aliases = new Map([
      ["main.server", 'game:GetService("ServerScriptService").server.main'],
      [
        "balloon/server/spawner",
        'game:GetService("ServerScriptService").server.balloon.spawner',
      ],
    ]);
    const result = compile(
      ...
      "main.server.ts",
      {
        warnLevel: "none",
        pathAliases: aliases,
        pathAliasFiles: new Set(aliases.keys()),
      }
    );
    expect(result.luau).toContain("require(script.Parent.balloon.spawner)");
  });

  test("a file does not inherit a same-named directory alias's tree position", () => {
    // src/shared.ts sits next to a directory-level mapping for src/shared/;
    // both produce the alias key "shared", but only a file-level entry pins
    // the importer's tree position, so the target stays absolute.
    const aliases = new Map([
    ...
    const result = compile(
      'import { config } from "./shared/config";\nexport const x = config;\n',
      "shared.ts",
      { warnLevel: "none", pathAliases: aliases }
    );
    expect(result.luau).toContain(
      'require(game:GetService("ReplicatedStorage").shared.config)'
    );
  });
});
